...
class AccountAnalyticLine(models.Model):
    ...
    error = fields.Boolean(default=False, tracking=True)
    pwork = fields.Boolean(default=False, tracking=True)
    ...
    def check_remnants_trips(self, employee_id, date):
        ...
        # Cerco se vi sono viaggi con il dipendente e data precisa ancora non nello stato "checked" o "cancel", nel caso mostro un avviso che non è possibile esportare le ore su Pwork in quanto vi sono ancora viaggi da gestire per quel dipendente.
            ...
            _logger.info(f"controllo i viaggi rimanenti di {record.employee_id.name} con id {employee_id} in data {date}")
            driver_id = self.env['hr.employee'].search_read([('id', '=', employee_id)], ['address_home_id'])
            _logger.info(f"Employee ID: {driver_id}")
            _logger.info(f"Employee ID: {driver_id[0]}")
            _logger.info(f"Employee ID: {driver_id[0]['address_home_id']}")
